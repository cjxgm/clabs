...
int main(void)
{
 ...
 while (1) {
  p = link_new(sizeof(double));
  if (scanf("%g", ((double *)(((void *)(p)) + sizeof(link_t)))) == (-1)) {
   free(p);
   break;
  }
  link_append((lnk)->prev, p);
 }
 ...
 while ((p = p->next) != lnk) {
  link_t * t = p;
  while ((t = t->next) != p)
   if (*(((double) *)(((void *)(t)) + sizeof(link_t))) < *(((double) *)(((void *)(p)) + sizeof(link_t)))) {
    link_swap(p, t);
    t = p;
   }
 }
